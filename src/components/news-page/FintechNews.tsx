...
export default function FintechNews() {
  ...
  useEffect(() => {
    const fetchNews = async () => {
      try {
        const response = await fetch(`/api/newsdata?q=fintech`);
  ...
  if (loading) {
    return (
      <Skeleton className="xl:max-w-[70%] px-4 xl:px-0 mx-auto h-[600px]" />
    );
  }
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
        {articles.map((article, index) => (
          <motion.div
            ...
            className="bg-[#07153b] rounded-lg overflow-hidden shadow-md hover:-translate-y-1 transition-transform duration-300 border border-slate-600"
          >
            <Link href={article.link} className="block">
              ...
              <div className="p-4">
                ...
                <p className="text-sm text-[#FFF] mb-3 line-clamp-3">
                  {article.description}
                </p>
                <div className="flex items-center justify-between text-xs text-[#FFF] border-t pt-3">
